...
index = faiss.read_index("vsd_rag_db/faiss_m3.index")
with open("vsd_rag_db/texts_m3.pkl", "rb") as f: texts = pickle.load(f)
with open("vsd_rag_db/metas_m3.pkl", "rb") as f: metas = pickle.load(f)
...
# 20 test queries with expected correct paper IDs
test_cases = [
    # === 精准命中：论文识别 ===
...
for i, tc in enumerate(test_cases):
    ...
    f_hit = len(f_top3 & expected)
    faiss_hits += f_hit
    faiss_total += 1

    # With reranker
    r_results = search(q, use_reranker=True) if reranker else f_results
    r_top3 = set(r["id"] for r in r_results[:3])
    r_hit = len(r_top3 & expected)
    rerank_hits += r_hit
    rerank_total += 1

    status = "✅" if r_hit > f_hit else ("=" if r_hit == f_hit else "❌")
    print(f"
[{i+1:2d}] {status} {q[:50]}...")
    print(f"    Expect: {tc['expect']}")
    print(f"    FAISS only:  {[r['id'] for r in f_results[:3]]}  (hit={f_hit})")
    if reranker:
        print(f"    + Reranker:  {[r['id'] for r in r_results[:3]]}  (hit={r_hit})")
...
if reranker:
    ...
    gain = rerank_hits - faiss_hits
    print(f"Improvement: +{gain} queries (+{gain/faiss_total:.0%})")
print("=" * 80)
